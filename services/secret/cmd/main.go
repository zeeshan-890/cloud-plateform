...
import (
	...
	"github.com/jp-cloud/secret/internal/handlers"
	"github.com/jp-cloud/secret/internal/store"
)

func main() {
	cfg, err := config.Load("8014")
	if err != nil {
		panic(err)
	}
	...
	masterSet := os.Getenv("SECRETS_MASTER_KEY") != ""
	...
	go func() {
		log.Info("secret listening", "port", cfg.HTTPPort, "master_key_configured", masterSet)
