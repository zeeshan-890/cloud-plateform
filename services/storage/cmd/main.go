...
func main() {
	...
	backend, err := miniox.NewFromEnv()
	if err != nil {
		log.Error("storage backend", "err", err)
		os.Exit(1)
	}
	...
	go func() {
		log.Info("storage listening", "port", cfg.HTTPPort, "mode", backend.Mode())
